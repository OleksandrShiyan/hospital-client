...
function UpdateRoom ({setUpdateRoom, updateRoom, setState, state}: updateRoomProps) {
    ...
    const onSubmit = (formData) => {
        ...
        const deleteRoomById = (deleteRoom) => {
            ...
            const newRoom = {...state[index.column].tasks.find(room => +room.id === +updateRoom.id), name: formData.name}
                                ...
                                <Field name="name">
                                    {({ input, meta }) => (
                                        <div className={style.createFormNameWrapper}>
                                            ...
                                            <input {...input} className={style.nameInput} type="text" placeholder="room name" />
                                            {meta.error && meta.touched && <span>{meta.error}</span>}
                                        </div>
                                    )}
                                </Field>
                            <button className={style.saveButton} id="edtSbmBtn" type="submit">Save</button>
                        </form>
                    )}/>
            </div>
            <div onClick={deleteEdit} className={style.modalCloser}></div>
        </div>
    )
