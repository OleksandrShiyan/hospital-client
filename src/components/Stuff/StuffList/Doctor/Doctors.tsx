...
const Doctors = ({
  ...
  const user = doctors?.find((doctor) => +doctor.id === +edit);

  console.log('doctors: ', doctors);

  return (
    <div>
      <button type="button" className={style.addDoctorButton} onClick={createNewDoctor}>
        Add new
      </button>
      {doctors?.map((doctor) => (
        <Doctor setDeleteId={setDeleteId} setEdit={setEdit} key={doctor.id} doctor={doctor} />
      ))}
      {edit && user ? (
