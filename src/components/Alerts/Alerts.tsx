...
const Alerts = () => {
  ...
  const [alerts, setAlerts] = useState<[AlertInterface]>([
    {
      id: 1, color: 'blue', textColor: 'red', name: 'cool',
    },
  ]);
  ...
  return (
    <div className={style.pageWrapper}>
      <div className={style.buttonWrapper}>
        <button type="button" className={style.button} onClick={crAlert}>
          Add new
        </button>
      </div>
      ...
      {edit && oneAlert ? <EditAlert setEdit={setEdit} alert={oneAlert} edit={edit} /> : null}
      {create ? <CreateAlert setCreate={setCreate} /> : null}
    </div>
  );
};
